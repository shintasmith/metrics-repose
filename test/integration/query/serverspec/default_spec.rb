...
describe file('/etc/repose/validator.cfg.xml') do
  ...
  its(:content) do
    should match %r{   <validator
      role="default"
      default="true"
      wadl="blueflood-query.wadl"
      dot-output="/tmp/blueflood-query.wadl.dot"
...
describe file('/etc/repose/container.cfg.xml') do
  it { should be_file }
  it { should be_mode 644 }
  its(:content) { should match(/<deployment-config content-body-read-limit="32768" connection-timeout="30000" read-timeout="30000" client-request-logging="true" >/) }
end

describe file('/etc/repose/dist-datastore.cfg.xml') do
  it { should be_file }
  it { should be_mode 644 }
  its(:content) { should match %r{<port port="9012" cluster="blueflood-query"/>} }
end
